...
import { applyUserFriend, delFriend as delUserFriend, getUsersBasicInfo, sendUserMsg } from '../../net/rpc';
...
export class UserDetail extends Widget {
    ...
    public getMore() {
        ...
        const blackText = this.blackPerson ? '移出黑名单' :'加入黑名单';
        if (this.props.userInfo.level !== VIP_LEVEL.VIP5) {
    ...
    public utilClick(ind: number) {
        this.props.showUtils = false;
        this.paint();
        if (this.props.userInfo.level !== VIP_LEVEL.VIP5 && !this.props.isFriend) {
            switch (ind) {
                case 0:// 加入或移出黑名单
                    this.blackList();
                    break;
